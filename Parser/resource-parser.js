...
⦿ sort=1/-1/x/参数规则, 按节点名 正/逆/随机/参数规则 排序
  ...
  ❖ sort=🇭🇰&gt;🇸🇬&gt;🇯🇵&gt;🇺🇸 , 靠前排序
  ❖ sort=IEPL<iplc<bgp !="-1" "";="" "weibo"="" "淘宝比价"="" "𝐬𝐬","𝐒𝐒","🅢🅢","🆂🆂","ⓢⓢ","🅂🅂","𝕊𝕊","ˢˢ"="" $emoji1="" $environment="" $index0="" $resource.content,="" $resource.info]="" $resource.tag="" $tag="" $type0="" &="" &")[0]="" &&="" *="" **="" +="" ,="" ------------------------------="" ----------------------------------------------------------="" 0="" 0;="" 0️⃣="" 1="" 1).replace(="" 1\①\❶\⓵\𝟙\¹\₁\𝟏\𝟷="" 2="" 2⃣️="" 3="" 3⃣️="" 4="" 5="" 6="" 7="" 8="" 9="" :="" ;="" =="" ?="" [link0,="" \$emoji="" \$index="" \$tag="" \$type="" app，直到出现解析器说明="" base64="new" base64code();="" beginning="" cdn="1," const="" content0="" content0,="" content0.split("\n")[0];="" dst="filter" ends="" escaperegexp="str" false;="" fastly.jsdelivr.net="" fcr="1/2/3," flow="2022-06-02:1000:54," force-cellular="" g,"node_emoji_flag_prefix").replace(="" g,"node_index_prefix")="" g,"node_tag_prefix").replace(="" g,"node_type_para_prefix").replace(="" gh="" github="" github.com="" hide="0," host规则，用于实现代理链="" html")="" https:="" in(hn)="" in,="" info=")[1].split(" inhn,="" js="" kop-xiao="" let="" link0="" link0.indexof("github.com")="" lite="" mark0='para.indexOf("#")' multi-interface="" multi-interface-balance="" npt="" ntf="0/1," ntf_flow="0;" number($environment.version.split("build")[1]):="" out(hn)="" out,="" outhn,="" para="/^(http|https)\:\/\//.test(link0)" para1='para.slice(para.indexOf("#")' para1.indexof("info=") != -1 ? para1.split(" perror="0" pinfo="mark0" policy="" profile="111" pset="regex1@policy1+regex2@policy2," ptn="" pull="" quantumultx="" quanx="" raw="" regex="" regex@newregex;="" regout,="" relay="目标策略名," rename="@「$tag34」," replace="(price)(.*)@$1_lite$2" resource_parser_url="https://raw.githubusercontent.com/KOP-XIAO/QuantumultX/master/Scripts/resource-parser.js" ssr="" subinfo]="[$resource.link," subtag="typeof" tag="" toraw(content0)="" true="" type="domain-set/rule/module/list/nodes" url-regex="" url-scheme="" var="" version="typeof" via="接口," via-interface="%TUN%" vmess="" “保留="" ∎="" ⚠️="" ⚠️⚠️如提示"没有自定义解析器"，请长按右下角图标后点击左侧刷新按钮，更新资源，后台退出="" ✈️="" ❖="" ➠="" ⟦进阶参数⟧:="" ⟦𝐟𝐢𝐥𝐭𝐞𝐫="" ⟦𝐫𝐞𝐰𝐫𝐢𝐭𝐞="" ⦿="" 一起使用，in(hn)="" 中="" 中的分流规则转换时,="" 为分流规则添加="" 为同一分流规则中不同关键词(允许正则表达式)指定不同策略组;="" 以防规则误删除="" 会优先执行;="" 传入一段="" 作为可操作参数，如="" 使用说明，="" 保留="" 关闭="" 其他参数="" 内容,="" 分流;="" 分流⟧="" 删除,="" 删除”主机名(𝒉𝒐𝒔𝒕𝒏𝒂𝒎𝒆);="" 占位符,="" 占位符，可用于="" 占位符，将节点的序号作为可操作参数，如="" 占位符，将节点类型(ss="" 占位符，将订阅的="" 参数,="" 参数说明:="" 参数，强制移动数据="" 及="" 可与="" 可尝试使用此参数强制指定="" 可接数字以单独给="" 可用于更为复杂的[过滤="" 同主机名="" 同时生效(⚠️="" 在quantumultx="" 对="" 将="" 将emoji(🇭🇰="" 将淘宝比价中脚本替换成="" 常用量="" 开启通知提示机场="" 当解析器未能正确识别类型时,="" 必须要加="" 慎用)="" 打开="" 批量将节点订阅转换为ip="" 把="" 数字样式="" 是否有參數需要解析="" 样式分别为="" 样式同下边的="" 根据关键词="" 正则保留="" 正则替换="" 正常使用部分，調試註釋此部分="" 流量信息(如有提供);="" 混合数据="" 添加="" 添加字母="" 版本(如有此版本的脚本)="" 版本号="" 生成策略组(默认"𝐒𝐡𝐚𝐰𝐧"策略组);="" 用于="" 用于直接指定策略组，或为="" 的="" 相关分流、重写规则;="" 示范:="" 禁用="" 禁用筛除的分流="" 等)作为可操作参数="" 等)作为可操作参数，如="" 等操作="" 类型="" 类型参数="" 类型配置中远程资源="" 编码的脚本,="" 脚本的地址转换成免翻墙="" 表示="" 解析器正常使用，調試註釋此部分="" 订阅到期时间:总流量:已用流量="" 说明:="" 请自行折腾正则表达式;="" 负载均衡="" 资源解析则默认”关闭“通知提示="" 资源解析器的提示通知="" 转成分流="" 转成重写时,="" 转换到重写,="" 转换成="" 通知参数="" 部分，加入="" 配置文件中[general]="" 重写="" 重写⟧="" 重写，默认方式为删除="" 重命名]="" 链接的沙雕情形="" 错误类型="" 防止参数中其它位置也存在"#"="" 隐藏参数="" 需求="" 非="" 靠后排序="" 默认“开启”通知提示,="" 默认将="" 𝐒𝐮𝐫𝐠𝐞="" 𝐟𝐢𝐥𝐭𝐞𝐫="" 𝐡𝐭𝐭𝐩𝐬:="" 𝐦𝐨𝐝𝐮𝐥𝐞="" 𝐦𝐨𝐝𝐮𝐥𝐞&𝗿𝘂𝗹𝗲-𝘀𝗲𝘁="" 𝐦𝐲𝐥𝐢𝐬𝐭#out="tb_price.js+wb_ad.js&amp;outhn=weibo" 𝐫𝐞𝐰𝐫𝐢𝐭𝐞="" 𝒉𝒐𝒔𝒕𝒏𝒂𝒎𝒆="" 𝗳𝗶𝗹𝘁𝗲𝗿="" 𝗿𝗲𝘄𝗿𝗶𝘁𝗲="" 𝗿𝘂𝗹𝗲-𝘀𝗲𝘁="" 𝘀𝗲𝗿𝘃𝗲𝗿="" 𝘀𝗳𝗶𝗹𝘁𝗲𝗿="" 𝘀𝗿𝗲𝗻𝗮𝗺𝗲,=""> str.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&amp;'); //处理特殊符号以便正则匹配使用
...
function titleCase(str) {
  ...
  for(var i = 0; i<newstr.length; ")="" ");="" ")].join("="" "「𝐬𝐬」"="" "「𝐬𝐬𝐫」"="" &&="" )="" ){="" +="" :="" [[item.split("tag=")[0]+
      " [item,="" function="" getnode_type(item,ind)="" i++){="" if(item.indexof("node_type_para_prefix")!="-1)" if(item.trim()!="" index_handle(item)="" item="" item.indexof("tag=")!=-1) {
    ind = !/^(0|1|2|3|4|5|6|7)$/.test(ind) ? 8 : ind
    typefix = {" items="item.map(item=" newstr.join("="" newstr[i]="newStr[i].slice(0,1).toUpperCase()" newstr[i].slice(1).tolowercase();="" return="" shadowsocks":["𝐬𝐬","𝐒𝐒","🅢🅢","🆂🆂","ⓢⓢ","🅂🅂","𝕊𝕊","ˢˢ","ss"],"shadowsocksr":["𝐬𝐬𝐫","𝐒𝐒𝐑","🅢🅢🅡","🆂🆂🆁","ⓢⓢⓡ","🅂🅂🅁","𝕊𝕊ℝ","ˢˢʳ","ssr"],"vmess":["𝐯𝐦𝐞𝐬𝐬","𝐕𝐌𝐄𝐒𝐒","🅥🅜🅔🅢🅢","🆅🅼🅴🆂🆂","ⓥⓜⓔⓢⓢ","🅅🄼🄴🅂🅂","𝕍𝕞𝕖𝕤𝕤","ᵛᵐᵉˢˢ","vmess"],"trojan":["𝐭𝐫𝐨𝐣𝐚𝐧","𝐓𝐑𝐎𝐉𝐀𝐍","🅣🅡🅞🅙🅐🅝","🆃🆁🅾🅹🅰🅽","ⓣⓡⓞⓙⓐⓝ","🅃🅁🄾🄹🄰🄽","𝕋𝕣𝕠𝕛𝕒𝕟","ᵀʳᵒʲᵃⁿ","trojan"],"http":["𝐡𝐭𝐭𝐩","𝐇𝐓𝐓𝐏","🅗🅣🅣🅟","🅷🆃🆃🅿","ⓗⓣⓣⓟ","🄷🅃🅃🄿","𝕙𝕥𝕥𝕡","ʰᵗᵗᵖ","http"],"socks5":["𝐬𝐨𝗰𝗸𝐬","𝐒𝐎𝐂𝐊𝐒","🅢🅞🅒🅚🅢","🆂🅾🅲🅺🆂","ⓢⓞⓒⓚⓢ","🅂🄾🄲🄺🅂","𝕤𝕠𝕔𝕜𝕤","ˢᵒᶜᵏˢ","socks"],"vless":["𝐯𝐥𝐞𝐬𝐬","𝐕𝐋𝐄𝐒𝐒","🅥🅛🅔🅢🅢","🆅🅻🅴🆂🆂","ⓥⓛⓔⓢⓢ","🅅🄻🄴🅂🅂","𝕍𝕝𝕖𝕤𝕤","ᵛˡᵉˢˢ","vless"]}="" tag=")[1]].join(" tp="" tp].join("="" type_handle(item)="" type_prefix(item)="" type_suffix(item)="" typefix='{"shadowsocks":"「𝐬𝐬」","vmess":"「𝐯𝐦𝐞𝐬𝐬」","trojan":"「𝐭𝐫𝐨𝐣𝐚𝐧」","http":"「𝐡𝐭𝐭𝐩」","vless":"「𝐯𝐥𝐞𝐬𝐬」"}' typefix["shadowsocks"]="" typefix["shadowsocksr"]="" {="" }="" 后缀="" 操作節點類型佔位符="" 类型前缀="" 节点序号占位符处理="" 获取类型="">item.trim()).filter(Boolean)
...
function ServerRelay(src,dst) {
  var rsts=[]
  for (var i=0; i<src.length; "host":"ip"="" "host-suffix,"+servera+","+dst="" "ip-cidr,"+servera+"="" ,")[0].trim()="" .test(servera)?="" 32,"+dst="" :="" :")[0].trim()="" =="ip" ?="" [a-z]$="" function="" i++)="" preg0='RegExp(".*js-file-line\".*?\&lt;\/td\' raw="" rawtest(cnt)="" return="" rst="type" rsts.join("\n")="" rsts.push(rst)="" servera='src[i].indexOf("-host")==-1?' src[i].split("=")[1].split(" src[i].split("-host")[1].split("=")[1].split(" type="/^[a-z]/.test(serverA)" var="" {="" ||="" }="" 用于某些奇葩用户不使用="" 链接的问题="">", "i")
  if (Preg0.test(cnt)) {
...
function policy_sets(cnt,para) {
  pcnt = para.split("+")
  cnt=cnt//.split("\n")
  for (i=0;i<pcnt.length;i++){ (pcnt[i].indexof("@")!="-1){" =="" cnt="cnt.map(item" console.log(pcnt[i])="" if=""> filter_set(item, pcnt[i]))
    }
  }
...
function SIP2QuanX (cnt) {
  ...
  ll =cnt.length
  nodes =[]
  for (i=0; i<ll; !="vmess" "="" ""="" "")).trim()).split("\u0000")[0];="" "")).trim().split("\u0000")[0];="" "").replace("http:="" "").split("?")[0]}="" "").split("?remark")[0].split("&remark")[0].split("?")[0]).trim()).split("\u0000")[0]="" "").trim()="" "").trim()).split("\u0000")[0])="" "":"over-tls='true"' "":cert="" "";="" ""}="" ")="" "):""="" ");="" ")[0].split("?")[0];="" ")[0];="" ",="" ".")="" ":"="" ";="" "aead='true";' "fast-open='false";' "http=" + server.split(" "http"="" "https"="" "i")="" "method="aes-128-gcm," "none"="" "not-supportted"="" "obfs="wss," "obfs-host=" + jsonl.host + " "obfs-uri=" + jsonl.path : " "over-tls='true":' "password=" + server.split(" "tag="+server.split(" "tls-host=" + subs.split(" "tls-verification='true";' "tls13='false"' "udp-relay='false";' "vmess="" "⚠️="" "已忽略以下="" "这是个点")="" #")[0].split("&")[0]="" #")[0].split("?")[0])="" #")[0].split("?")[0];="" $notify(="" $notify("⚠️="" $notify(node)="" &="" &")[0])="" &")[0]);="" &")[0].split("\n")[0]).split("\n")[0].trim()="" &")[0].trim()="" &&="" ((jsonl.net="tcp" (e)="" (host.indexof("\"host\"")!="-1" (jsonl.net="" (jsonl.tls="tls" (obfs="" (obfs.indexof("obfs='over-tls")' (pntf0!="0)" (server="" (server.indexof("@")="-1" (server.indexof("@")!="-1)" (server.indexof("obfs='http")' (subs.indexof("#")="-1)" (subs.indexof("alterid=") != -1) {
    caead = Number(subs.split(" (tls="" )="" ))="" ){="" +="" +[ip,pwd,="" +caead+="" +cnti.remarks="" ,="" ,")[0]="" ,")[4].split("\"")[1]="" -1?="" .").map(todot);="" .split("tag=")[1]
  if (!Preg0.test(cnt)) {
    return content
  } else {
    RegoutList.push(cnt)
...
function Scheck(content, param) {
    name = content.replace(/tag\s*\=/g," :="" ;="" =="" ?="" @")[0]+="" @")[0].split(":")[1]="" @")[1]="" \"")[1]="" \\\.="" aead="" alterid=")[1].split(" b64(ipport)="" b64(ipport)?peer="xxx#server-remarks" base64.decode(subs.replace("https:="" caead="ss.aid" caead,="" catch="" cert="Pcert0;" cert,="" checkpara="(item)" cnt="content" cnt.replace(="" cnti="cnt[i]" cnti.method="" cnti.password="" cnti.plugin_opts.replace(";",="" cnti.server_port="" console.log("v2="" console.log(nodes)="" const="" decodeuricomponent(server.split("#")[1]);="" decodeuricomponent(server.split("remarks=")[1].split(" decodeuricomponent(ss.ps);="" decodeuricomponent(subs.split("&obfsparam=")[1].split(" decodeuricomponent(subs.split("&path=")[1].split(" dot2(cnt)="" else="" fast-open="false," fobfs(jsonl,="" function="" g,="" grpc|h2="" h2="" host="" host.indexof("{")!="-1)" host0="" host0)="" hps2qx(subs,="" http="" https="" https:="" i")="" i++)="" if="" if(obfs="" if(pdbg="1)" if(pdbg)="" ip="vmess=" ipport="socks5=" json.stringify(jsonl))="" jsonl.host="" jsonl.net="none" jsonl.net,="" jsonl.path="" jsonl.tls="tls" jsonl.type="" listh='Base64.decode(cnt.split("socks://")[1].split("#")[0].split("?")[0])' mtd="method=aes-128-gcm" mtd,="" node="" nodes.join("\n")="" nodes.push(node)="" nss="[];" nss.push(ip,="" nss.push(ipport,="" obfs="" obfs,="" obfs-header='\"Host:")[1].split("[")[0].trim()' obfs-host=" + jsonl.host : " obfs-path=")[1].split(" obfs-uri='/";' obfs.indexof("obfs='wss")' obfs0="obfs=http" obfsi="[];" obfsi.join(",="" obfsi.push(obfs0="" obfsi.push(obfs0,="" ouri='subs.indexOf("&amp;path=")' out="" over-tls="" password="pwd," pcert0,="" pdrop="1" peer=")[1].split(" perror="1" preg0="RegExp(Preg," ptfo,="" ptls13)="" pudp,="" pwd="password=" pwd+="" pwd,="" quantumult="" quanx="" qx="" regex(content)="" rename中)="" return="" s5r2qx(cnt,tlsp="false" server='String(Base64.decode(subs.replace("vmess://",' server.split("=")[0]
  var tfo = subs.indexOf(" server.split(",")[1].trim()="" server.split(",")[2].trim()="" server.split(":")[0];="" server.split("@")[0].split(":")[1];="" server.split("@")[1]="" server.split("@")[1].split("#")[0].split("="" shadowrocket="" socks="" ss="JSON.parse(server);" ss.add="" ss.aid="" ss.id;="" ss.port;="" ss.ps;="" subs.indexof("@")!="-1)" subs.indexof("allowinsecure='1")' subs.split("#")[1]="" subs.split("obfs=")[1].split(" tag=")[1].toUpperCase()
    param = param ? param.map(Dot2) : param // 对符号.的特殊处理
    if (param) {
        var flag = 0;
        for (var i = 0; i &lt; param.length; i++) {
            var params = param[i].split(" tag)="" tag);="" tag].filter(boolean).join(",="" tcert="" tfo="Ptfo" tfo,="" thost='subs.indexOf("peer=")!=' thost,="" tls='tlsp=="false"?' tls,="" tls-host=")[1].split(" tls-verification="true," tls13="" tls13);="" tls13,="" tls13;="" todot(cnt)="" try="" type='subs.indexOf("https://")!=-1?' type",subs)="" udp="Pudp" udp,="" udp-relay="false," uname="username=" uname,="" uri='/")' uri0='uri0.indexOf("uri=/")!=-1' uri0)="" uri0);="" uri0:uri0.replace("uri="," username="name," v2qx(subs,="" v2rayn="" var="" vmess="" vq2qx(subs,="" vr2qx(subs,="" ws,wss="" x="" {="" {$notify("shadowrocket-vmess-uri","..",subs+"\n\n"+server)}="" {$notify("vmess-uri","",json.stringify(ss))}="" {return="" ||="" }="" 不支持="" 不支持该类型节点",="" 不需要反馈的类型="" 参数="" 存在peers参数时="" 完整内容匹配="" 对.的特殊处理(in="" 暂不支持该类型节点",="" 格式="" 格式的="" 正则筛选,="" 等类型="" 类型="" 节点",subs)="" 转换="" 转换成="" 过滤掉="" 这是个点="" 选项="" 部分无节点名的情况=""> name.indexOf(item.toUpperCase()) != -1;
...
function Sort_KWD (cnt,strs) {
  ...
  for (var i =0; i<cnt.length !="1){" (flag="" (var="" ;="" =="-1?" arr="PsortX" arr.map(item="" arr[j].push(cnt[i])="" arr[strlist.length].push(cnt[i])="" break="" console.log(arr)="" flag="1" for="" i++)="" if="" if(regj[j].test(cnt[i]))="" j="0;" j++){="" j<strlist.length="" {="" }="" 不匹配项=""> item.sort(ToTagR)):arr
...
function MixArr(cnt,dir){
  var cnt0=[]
  for (i=0; i<cnt.length-1; !="-1)" "="" ""="" "")="" "",="" "":="" ")+cnt2="" ")[0]="" ",="" "gm"),="" "gm");="" "gmi")="" "tag=" + name + tail
        }
      return nserver
    } else {
      return server
...
function RenameScript(servers, script) {
    $notify(" $="Tools().rename;" &&="" ((pemoji="1" (j="0;" (nname="" (oname="" (server.indexof("tag=") != -1) {
        hd = server.split(" )="" +="" ,")[0].trim()="" -="" 2)).split(".")="" :="" <="" =="1?" ?="" @")[1]="" cnt="" cnt0='content.split("tag=")[0]' cnt0.concat(cnt[cnt.length-1].sort(totag)):(cnt[cnt.length-1].sort(totagr)).concat(cnt0)="" cnt0.concat(cnt[i]):cnt0.concat(cnt[cnt.length-2-i])="" cnt1='content.split("tag=")[1].split(",")[0]' cnt2='content.split("tag=")[1].split(",").length==1' console.log(dir)="" const="" content='content.replace(/tag\s*\=\s*/,"tag=")' content.split(cnt1)[1]="" decodeuricomponent(prn[i].split("@")[0])="" decodeuricomponent(prn[i].split("@")[1])="" del="new" delreg="RegExp(delreg," delreg(content)="" else="" emoji，有则删除旧有="" eval(script);="" extract="" for="" function="" hh="Dot2(oname.slice(0," hh.length;="" i)="" i++){="" if="" j="" j++)="" name="name" name.replace(name.split("="" name.split("="" name.trim()="" nemoji="" newnames="rename(nodes);" nn="escapeRegExp(ToDot(hh[j]))" nname="" nname)="" nodes="" nserver="hd" oname="" oname)="" oname.indexof("☠️")="" oname.length="" pemoji="2)" prn[i].split("@")[0];="" prn[i].split("@")[1];="" prname="" regexp(nn,="" regexp(rn,="" rename="" rename(str)="" return="" rn="escapeRegExp(oname)" script);="" server="" server.split("tag=")[1].split(name)[1]
        for (var i = 0; i &lt; Prn.length; i++) {
            nname = Prn[i].split(" servers.map((s,="" tag=")[1].split(" tags="" tail='server.split("tag=")[1].split(",").length' try="" var="" {="" ||="" }="" 仅有@时，删除@符号="" 删除特定字符，多字符用.连接="" 判断是否有重复="" 前缀="" 后缀="" 启用脚本进行重命名",="" 正则删除节点名内的字符="" 符号.的特殊处理="" 节点重命名="" 部分="" 重命名="" 🤖=""> s.split("tag=")[0] + "tag=" + newNames[i]);
    } catch (err) {
...
function Clash2QX(cnt) {
  ...
  var nodelist=[]
  var node=""
  for (i=0; i<nl; !="-1)" ""="" ")="" "aes-128-gcm"="" "fast-open='false"' "obfs='ws"' "obfs-host=" + cnt[" "obfs-uri=" + cnt[" "tls-verification='false"}' "udp-over-tcp='false"' "udp-relay='false"' $notify(`⚠️该节点解析错误,="" $notify(`⚠️错误内容如下`,`可复制错误内容到反馈="" &&="" (="" (cnt.network="http" (cnt.tls){="" (cnt["obfsparam"])="" (e)="" (obfs="obfs=wss" (obfs.indexof("websocket")="" (typecc="socks5" (typeof(cnt["protocol-param"])="object" )="" ){="" +="" +[ipt,="" +cnt.name.replace(="" :="" =="1?" ?="" \\u.+?\s{1}="" bot`,json.stringify(node)+"\n\n"+e)="" bot`,json.stringify(node),bug_link="" cert="Pcert0" cert,="" clash="" cnt.cipher="" cnt.obfs="" cnt.password="" cnt.protocol="" cnt.tls)="" cnt.uuid="" cnt["obfs-param"]="" cnt["obfs-param"])="" cnt["obfsparam"]="" cnt["protocol-param"]="" console.log(node)="" console.log(obfs)="" console.log(typeof(cnt["protocol-param"]))="" const="" css2qx(cnt)="" cssr2qx(cnt)="" cv2qx(cnt)="" else="" function="" g,"")="" gi,"="" gi,"")="" i++){="" if="" if(cnt["protocolparam"])="" ipt='cnt.server+":"+cnt.port' json.stringify(cnt["protocol-param"]).replace(="" mtd="method=" mtd,="" node="" nodelist.join("\n")="" nodelist.push(node)="" obfs="obfs=over-tls" obfs,="" ohost="obfs-host=" ohost,="" ouri='cnt["plugin-opts"].path?' ouri,="" phost="getValue(()=" plugin-opts"].host="" plugin-opts"].host:""="" plugin-opts"].mode="" plugin-opts"].path:="" ppara="ssr-protocol-param=" ppara,="" prot="ssr-protocol=" prot,="" pwd="password=" pwd,="" return="" server="" ss="" ssr="" tag="tag=" tag].filter(boolean).join(",="" tfo="cnt.tfo" tfo,="" tls="" try{="" type="" typecc="node.type" udp="cnt.udp" udp,="" uot='cnt["udp-over-tcp"]' uot,="" verification="" vmess="" xtfo(node,ptfo0)="" xudp(node,pudp0)="" {="" {|}|\s|"="" }="" }catch="" 暂不支持="" 暂时已忽略处理`,`可点击通知并发送链接反馈至="">cnt["ws-opts"]["headers"]["Host"]) 
...
</nl;></cnt.length-1;></cnt.length></ll;></pcnt.length;i++){></src.length;></newstr.length;></iplc<bgp>