...
⦿ sort=1/-1/x/参数规则, 按节点名 正/逆/随机/参数规则 排序
  ...
  ❖ sort=🇭🇰&gt;🇸🇬&gt;🇯🇵&gt;🇺🇸 , 靠前排序
  ❖ sort=IEPL<iplc :="" app="" base64="" base64code="" cdn="1," const="" content0="" content0.split="" dst="rewrite/filter，分别为将" escaperegexp="str" false="" fastly.jsdelivr.net="" fcr="1/2/3," flow="2022-06-02:1000:54," force-cellular="" github="" hide="0," html="" https:="" in="" info="1," inhn="" js="" let="" link0="" link0.indexof="" lite="" mark0='para.indexOf("#")' ntf="0/1," ntf_flow="0;" number="" out="" outhn="" para="/^(http|https)\:\/\//.test(link0)" para1='para.slice(para.indexOf("#")' para1.indexof="" para1.split="" perror="0" pinfo="mark0" policy="" profile="111" pset="regex1@policy1+regex2@policy2," ptn="" quanx="" raw="" regex="" relay="目标策略名," rename="" replace="" resource_parser_url="https://raw.githubusercontent.com/KOP-XIAO/QuantumultX/master/Scripts/resource-parser.js" subinfo="" subtag="typeof" tag="" toraw="" true="" type="domain-set/rule/module/list/nodes" url-regex="" url-scheme="" var="" version="typeof" via="接口," via-interface=""> str.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&amp;'); //处理特殊符号以便正则匹配使用
...
function titleCase(str) {
  ...
  for(var i = 0; i<newstr.length :="" function="" getnode_type="" i="" if="" ind="!/^(0|1|2|3|4|5|6|7)$/.test(ind)" index_handle="" item='item.replace(/node_type_para_prefix(\d{0,1})/g,getnode_type(item,item.split("node_type_para_prefix")[1][0]))' item.indexof="" item.split="" items="item.map(item=" newstr="" newstr.join="" return="" tp='typefix[item.split("=")[0].trim()]' type_handle="" type_prefix="" type_suffix="" typefix='{"shadowsocks":"「𝐬𝐬」","vmess":"「𝐯𝐦𝐞𝐬𝐬」","trojan":"「𝐭𝐫𝐨𝐣𝐚𝐧」","http":"「𝐡𝐭𝐭𝐩」","socks5":"「𝐬𝐨𝗰𝗸𝐬」","vless":"「𝐯𝐥𝐞𝐬𝐬」"}'>item.trim()).filter(Boolean)
...
function ServerRelay(src,dst) {
  var rsts=[]
  for (var i=0; i<src.length :="" function="" i="" preg0='RegExp(".*js-file-line\".*?\&lt;\/td\' raw="" rawtest="" return="" rst="type" rsts.join="" rsts.push="" servera='src[i].indexOf("-host")==-1?' src="" type="/^[a-z]/.test(serverA)" var="">", "i")
  if (Preg0.test(cnt)) {
...
function policy_sets(cnt,para) {
  pcnt = para.split("+")
  cnt=cnt//.split("\n")
  for (i=0;i<pcnt.length cnt="cnt.map(item" console.log="" if=""> filter_set(item, pcnt[i]))
    }
  }
...
function SIP2QuanX (cnt) {
  ...
  ll =cnt.length
  nodes =[]
  for (i=0; i<ll :="" base64.decode="" caead="aead=false, " catch="" cert="Pcert0" checkpara="(item)" cnt="cnt" cnt.replace="" cnti="cnt[i]" cnti.method="" cnti.password="" cnti.plugin_opts.replace="" cnti.server_port="" const="" content="" decodeuricomponent="" dot2="" else="" fast-open="false," flag="0;" fobfs="" for="" function="" grpc="" h2="" host='server.indexOf("tls-host")' host.indexof="" host0="jsonl.host" hps2qx="" http="" https:="" i="" if="" ip="cnti.server" ipport="http=" json.stringify="" jsonl.host="" jsonl.net="=" jsonl.path="" jsonl.tls="=" jsonl.type="=" listh='Base64.decode(cnt.split("socks://")[1].split("#")[0].split("?")[0])' mtd="method=" name='content.replace(/tag\s*\=/g,"tag=").split("tag=")[1].toUpperCase()' node="shadowsocks= " nodes.join="" nodes.push="" nss="[]" nss.push="" obfs="cnti.plugin_opts?" obfs-host=" + jsonl.host : " obfs.indexof="" obfs0="obfs=wss, " obfsi="[];" obfsi.join="" obfsi.push="" ouri='subs.indexOf("&amp;path=")' over-tls="true," param="" param.length="" param.map="" params='param[i].split(".").map(ToDot);' password="pwd," pcert0="" pdrop="0" perror="1" preg0="RegExp(Preg," ptfo="" ptls13="" pudp="" pwd="password=" quantumult="" quanx="" qx='nss.filter(Boolean).join(",");' regex="" regexout="" regoutlist.push="" rename="" return="" s5r2qx="" scheck="" server='subs.replace("https://",' server.split="" socks="" ss="JSON.parse(server);" ss.add="" ss.aid="" ss.id="" ss.port="" ss.ps="" subs.indexof="" subs.split="" tag="tag=" tcert="cert" tfo="Ptfo" thost='subs.indexOf("peer=")!=' tls="type" tls-host="example.com," tls-verification="true," tls13="true," todot="" try="" type='subs.indexOf("https://")!=-1?' udp="Pudp" udp-relay="false," uname="username=" undefined="" uri="" uri0="jsonl.path" uri0:uri0.replace="" username="name," v2qx="" var="" vmess="" vq2qx="" vr2qx="" ws="" x=""> name.indexOf(item.toUpperCase()) != -1;
...
function Sort_KWD (cnt,strs) {
  ...
  for (var i =0; i<cnt.length arr="" arr.map="" break="" flag="0" for="" i="" if="" j="0;"> item.sort(ToTagR)):arr
...
function MixArr(cnt,dir){
  var cnt0=[]
  for (i=0; i<cnt.length-1 :="" cnt="cnt0" cnt0="dir" cnt0.concat="" cnt1='content.split("tag=")[1].split(",")[0]' cnt1.replace="" cnt2='content.split("tag=")[1].split(",").length==1' const="" content='content.replace(/tag\s*\=\s*/,"tag=")' content.split="" decodeuricomponent="" del="new" delreg="" else="" emoji="" eval="" extract="" for="" function="" hd='server.split("tag=")[0]' hh="Dot2(oname.slice(0," hh.length="" i="" if="" j="" name="" name.split="" name.trim="" nemoji="emoji_del(name)" newnames="rename(nodes);" nn="escapeRegExp(ToDot(hh[j]))" nname='Prn[i].split("@")[1]' nodes="Tools().getNodeInfo(servers);" nserver="hd" oname='Prn[i].split("@")[0]' oname.indexof="" oname.length="" pemoji="=" prn="" prn.length="" prname="" regexp="" rename="" renamescript="" return="" rn="escapeRegExp(oname)" script="" server="str;" server.split="" servers.map="" tags="" tail='server.split("tag=")[1].split(",").length' tools="" try="" var=""> s.split("tag=")[0] + "tag=" + newNames[i]);
    } catch (err) {
...
function Clash2QX(cnt) {
  ...
  var nodelist=[]
  var node=""
  for (i=0; i<nl :="" bot="" cert="" cnt="" cnt.cipher="" cnt.obfs="" cnt.password="" cnt.protocol="" cnt.tls="" cnt.uuid="" console.log="" const="" css2qx="" cssr2qx="" cv2qx="" else="" function="" i="" if="" ipt='cnt.server+":"+cnt.port' json.stringify="" mtd="method=" node="bb[i]" nodelist.join="" nodelist.push="" obfs='cnt["plugin-opts"]' ohost='cnt["plugin-opts"]' ouri="" phost="getValue(()=" ppara="" prot="ssr-protocol=" pwd="password=" return="" server="" ss="" ssr="" tag="tag=" tfo="cnt.tfo" tls="" try="" type="" typecc="node.type" udp="cnt.udp" uot='cnt["udp-over-tcp"]' verification="" vmess="" xtfo="" xudp="">cnt["ws-opts"]["headers"]["Host"]) 
...
</nl></cnt.length-1></cnt.length></ll></pcnt.length></src.length></newstr.length></iplc></p></body></html>